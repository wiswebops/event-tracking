...
$.extend(SVGWrapper.prototype, {
	...
	_args: function(values, names, optSettings, optData) {
		names.splice(0, 0, 'parent');
		names.splice(names.length, 0, 'settings');
		names.splice(names.length, 0, 'ExtraData');
		...
		if (optData) {
			$.each(optData, function(i, value) {
			  
				if (typeof args[value] == 'object') {
				     
					args.ExtraData=args[value];
					args[value] = null;
				}
	...
	/* Draw a rectangle.
	   ...
	   @param  settings  (object) additional settings for the shape (optional)
	   @param  data  (object) additional settings for the shape (optional)
	   @return  (element) the new shape node */
	rect: function(parent, x, y, width, height, rx, ry, settings, datax) {
		var args = this._args(arguments, ['x', 'y', 'width', 'height', 'rx', 'ry'], ['rx'],['ry']);
		return this._makeNode(args.parent, 'rect', $.extend(
			{x: args.x, y: args.y, width: args.width, height: args.height},
			(args.rx ? {rx: args.rx, ry: args.ry} : {}), args.settings || {}, args.ExtraData || {}));
	},
